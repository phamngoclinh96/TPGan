...
flags.DEFINE_integer("train_size", 128, "The size of train images [np.inf]")
...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.images_with_code = tf.placeholder(tf.float32,
                                               [self.batch_size] + [self.output_size, self.output_size, CHANNEL],
                                               name='images_with_code')
        self.sample_images = tf.placeholder(tf.float32,
                                            [self.test_batch_size] + [self.output_size, self.output_size, CHANNEL],
                                            name='sample_images')
        ...
        self.labels = tf.placeholder(tf.float32, [self.batch_size] + [self.output_size, self.output_size, 3],
                                     name='label_images')
        ...
        self.G_eyer_sam, self.c_eyer_sam = self.partRotator(tf.concat([self.eyer_sam, self.eyel_sam], axis=3),
                                                            "PartRotator_eyer", reuse=True)
        ...
        self.d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels= self.D_logits,logits= tf.ones_like(self.D* 0.9) ))
        self.d_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels= self.D_logits_,logits= tf.zeros_like(self.D_)))
        self.g_loss_adver = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels= self.D_logits_,logits= tf.ones_like(self.D_) * 0.9))
        ...
        self.g_loss = L1_1_W * (self.weightedErrL1 + SYM_W * self.symErrL1) + L1_2_W * (
                self.weightedErrL2 + SYM_W * self.symErrL2) \
    ...
    def train(self, config):
        ...
        data = MultiPIE(LOAD_60_LABEL=LOAD_60_LABEL, GENERATE_MASK=USE_MASK, RANDOM_VERIFY=RANDOM_VERIFY,
                        MIRROR_TO_ONE_SIDE=True, source=self.testingphase)
        ...
        sample_labels, sample_leyel, sample_leyer, sample_lnose, sample_lmouth, sample_iden = data.test_batch(
            self.test_batch_size * self.sample_run_num)
        if not self.testing:
            sample_imagesT, filenamesT, sample_eyelT, sample_eyerT, sample_noseT, sample_mouthT, \
            sample_labelsT, sample_leyelT, sample_leyerT, sample_lnoseT, sample_lmouthT, sample_idenT = data.test_batch(
                self.test_batch_size * self.sample_run_num * RANK_MUL, Pose=RANGE)
            if WITHOUT_CODEMAP:
                sample_images = sample_images[..., 0:3]
            # append loss log to file
            self.f = open(self.logfile, mode='a')
            self.f.write(
                '----' + strftime("%a, %d %b %Y %H:%M:%S +0000", localtime()) + ' BEGINS----MODE:' + MODE + '-----\n')
            ...
            for epoch in xrange(config.epoch):
                ...
                for idx in xrange(0, batch_idxs):
                    ...
                    batch_eyel_label, batch_eyer_label, batch_nose_label, batch_mouth_label \
                        = data.next_image_and_label_mask_batch(self.batch_size, imageRange=RANGE,
                                                               imageRangeLow=RANGE_LOW)
                    ...
                    if (counter % 5 == 0):
                        self.evaluate(epoch, idx, batch_idxs, start_time, 'train',
                                      ...
                                      batch_labels, batch_eyel_label, batch_eyer_label, batch_nose_label,
                    ...
                    if np.mod(counter, self.sample_interval) == self.sample_interval - 1:
                        for i in range(self.sample_run_num):
                            ...
                            savedtest = save_images(
                                currentBatchSamples if WITHOUT_CODEMAP else currentBatchSamples[..., 0:3], [100, 100],
                                './{}/{:02d}_{:04d}/train{}_'.format(config.sample_dir, epoch, idx, i), suffix='')
                            savedoutput = save_images(samples[5], [100, 100],
                                                      './{}/{:02d}_{:04d}/train{}_'.format(config.sample_dir, epoch,
                                                                                           idx, i), suffix='_128')
                            ...
                            savedoutput = save_images(samples[7], [100, 100],
                                                      './{}/{:02d}_{:04d}/train{}_'.format(config.sample_dir, epoch,
                                                                                           idx, i), suffix='_32')
                        print(
                            "[{} completed{} and saved {}.]".format(config.sample_dir, savedtest * self.sample_run_num,
                                                                    savedoutput * self.sample_run_num))
                        ...
                        for i in range(self.sample_run_num * RANK_MUL):
                            currentBatchSamples = sample_imagesT[
                            ...
                            currentBatchMouth = sample_mouthT[i * self.test_batch_size:(i + 1) * self.test_batch_size,
                                                ...]
                            currentBatchLSamples = sample_labelsT[
                                                   i * self.test_batch_size:(i + 1) * self.test_batch_size, ...]
                            ...
                            if i % (4 * RANK_MUL) == 0:
                                self.evaluate(epoch, idx, batch_idxs, start_time, 'test',
                                              currentBatchSamples, currentBatchEyel, currentBatchEyer, currentBatchNose,
                                              currentBatchMouth,
                                              currentBatchLSamples, currentBatchLEyel, currentBatchLEyer,
                                              currentBatchLNose, currentBatchLMouth, currentBatchIden);
                            samples = self.sess.run(
                                self.sample_generator,
                                feed_dict={self.sample_images: currentBatchSamples,
                                           self.eyel_sam: currentBatchEyel,
                            ...
                            for k in range(samplevectors.shape[0]):
                                ...
                                f.close()
                        listfid.close()
                        print("[{}  saved {} feats. calling comparision..]".format(savedir,
                                                                                   self.test_batch_size * self.sample_run_num))
                        output, err = subprocess.Popen('./evaluation_rank.sh', stdout=subprocess.PIPE,
                                                       shell=True).communicate()
                        tobePrint = '-------!' + ''.join(
                            [rank for rank in output.splitlines() if rank.startswith('Rank-1')]) + '!-------'
                        print(err, tobePrint)
            ...
            for i in range(batchnum):
                ...
                if self.testimg:  # Save images
                    ...
                    savedtest += save_images(colorgt, [128, 128],
                                             './{}/'.format(sample_dir), isOutput=False,
                                             filelist=filenames[ind[0]:ind[1]])
                    savedoutput += save_images(samples[5], [128, 128],
                                               './{}/'.format(sample_dir), isOutput=True,
                                               filelist=filenames[ind[0]:ind[1]])
    ...
    def FeaturePredict(self, featvec, reuse=False):
        with tf.variable_scope("FeaturePredict") as scope:
            if reuse:
                scope.reuse_variables()
            identitylogits = \
                linear(Dropout(featvec, keep_prob=0.3, is_training=not self.testing), output_size=340,
                       scope='idenLinear',
    ...
    def decoder(self, feat128, feat64, feat32, feat16, feat8, featvec,
        ...
        with tf.variable_scope(name) as scope:
            ...
            initial_8 = relu(tf.reshape(
                linear(initial_all, output_size=8 * 8 * self.gf_dim, scope='initial8', bias_start=0.1, with_w=True)[0],
                [batch_size, 8, 8, self.gf_dim]))
            initial_32 = relu(
                deconv2d(initial_8, [batch_size, 32, 32, self.gf_dim // 2], d_h=4, d_w=4, name="initial32"))
            ...
            reconstruct16_deconv = relu(
                batch_norm(deconv2d(reconstruct8, [batch_size, 16, 16, self.gf_dim * 8], name="g_deconv16"),
                           name="g_bnd1"))
            before_select16 = resblock(feat16, name="select16_res_1")
            reconstruct16 = resblock(
                resblock(tf.concat([reconstruct16_deconv, before_select16], 3), name="dec16_res1"),
                name="dec16_res2")

            reconstruct32_deconv = relu(
                batch_norm(deconv2d(reconstruct16, [batch_size, 32, 32, self.gf_dim * 4], name="g_deconv32"),
                           name="g_bnd2"))
            before_select32 = resblock(tf.concat([feat32, g32_images_with_code, initial_32], 3),
                                       name="select32_res_1")
            reconstruct32 = resblock(
                resblock(tf.concat([reconstruct32_deconv, before_select32], 3), name="dec32_res1"),
                name="dec32_res2")
            img32 = tf.nn.tanh(conv2d(reconstruct32, 3, d_h=1, d_w=1, name="check_img32"))

            reconstruct64_deconv = relu(
                batch_norm(deconv2d(reconstruct32, [batch_size, 64, 64, self.gf_dim * 2], name="g_deconv64"),
                           name="g_bnd3"))
            before_select64 = resblock(tf.concat([feat64, g64_images_with_code, initial_64], 3), k_h=5, k_w=5,
                                       name="select64_res_1")
            ...
            reconstruct128_deconv = relu(
                batch_norm(deconv2d(reconstruct64, [batch_size, 128, 128, self.gf_dim], name="g_deconv128"),
                           name="g_bnd4"))
            before_select128 = resblock(tf.concat([feat128, initial_128, g128_images_with_code], 3), k_h=7, k_w=7,
            ...
            reconstruct128_1 = lrelu(
                batch_norm(conv2d(reconstruct128, self.gf_dim, k_h=5, k_w=5, d_h=1, d_w=1, name="recon128_conv"),
                           name="recon128_bnc"))
            reconstruct128_1_r = resblock(reconstruct128_1, name="dec128_res2")
            reconstruct128_2 = lrelu(
                batch_norm(conv2d(reconstruct128_1_r, self.gf_dim / 2, d_h=1, d_w=1, name="recon128_conv2"),
                           name="recon128_bnc2"))
    ...
    def partRotator(self, images, name, batch_size=10, reuse=False):
        ...
        with tf.variable_scope(name) as scope:
            ...
            d1 = lrelu(
                batch_norm(deconv2d(c3r2, [shape[0], shape[1] * 2, shape[2] * 2, self.gf_dim * 4], name="p_deconv1"),
                           name="p_bnd1"))
            # up1
            after_select_d1 = lrelu(
                batch_norm(conv2d(tf.concat([d1, c2r], axis=3), self.gf_dim * 4, d_h=1, d_w=1, name="p_deconv1_s"),
                           name="p_bnd1_s"))
            d1_r = resblock(after_select_d1, name="p_deconv1_res")
            d2 = lrelu(
                batch_norm(deconv2d(d1_r, [shape[0], shape[1] * 4, shape[2] * 4, self.gf_dim * 2], name="p_deconv2"),
                           name="p_bnd2"))
            # up2
            after_select_d2 = lrelu(
                batch_norm(conv2d(tf.concat([d2, c1r], axis=3), self.gf_dim * 2, d_h=1, d_w=1, name="p_deconv2_s"),
                           name="p_bnd2_s"))
            d2_r = resblock(after_select_d2, name="p_deconv2_res")
            d3 = lrelu(batch_norm(deconv2d(d2_r, [shape[0], shape[1] * 8, shape[2] * 8, self.gf_dim], name="p_deconv3"),
                                  name="p_bnd3"))
            # up3
            after_select_d3 = lrelu(
                batch_norm(conv2d(tf.concat([d3, c0r], axis=3), self.gf_dim, d_h=1, d_w=1, name="p_deconv3_s"),
                           name="p_bnd3_s"))
    ...
    def evaluate(self, epoch, idx, batch_idxs, start_time, mode,
        ...
        if 'f' in MODE:
            ...
            errDv = 0
        err_total_G = L1_1_W * (errG_L + errG_sym * SYM_W) + L1_2_W * (errG_L2 + errG2_sym * SYM_W) + L1_3_W * (
                errG_L3 + errG3_sym * SYM_W) \
        ...
        tobePrint = "%s Epo[%2d][%4d/%4d][t%4.2f] d_l:%.4f" % (
            MODE + 'T' if mode == 'test' else '', epoch, idx, batch_idxs, time.time() - start_time, errD)
        ...
        tobePrint += "g_l:%.0f gL1:%.0f(sym:%.0f) gL2:%.0f(sym:%.0f) gL3:%.0f(sym:%.0f) gadv:%.4f dv_l:%.2f iden:%.4f, tv:%.0f " \
                     % (
                         err_total_G, errG_L, errG_sym, errG_L2, errG2_sym, errG_L3, errG3_sym, errG_adver, errDv,
                         erriden,
                         errtv)
        if 'f' in MODE:
